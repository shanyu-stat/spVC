...
# k, ".RData")
file.name <- "simulated-5000.RData"
load(file.name)
...
# model fitting ----
res.model1 <- test.spVC(Y = sample[1:5000, ], X = X[, -1], S, V, Tr,
                        para.cores = 8,
                        filter.min.nonzero = -1)
res.model2 <- test.spVC(Y = sample[5001:10000, ], X = X[, -1], S, V, Tr,
                        para.cores = 8,
                        filter.min.nonzero = -1)
res.model3 <- test.spVC(Y = sample[10001:15000, ], X = X[, -1], S, V, Tr,
                        para.cores = 8,
                        filter.min.nonzero = -1)
res.model4 <- test.spVC(Y = sample[15001:20000, ], X = X[, -1], S, V, Tr,
                        para.cores = 8,
                        filter.min.nonzero = -1)

save(res.model1, res.model2, res.model3, res.model4, file = paste0("test-5000.RData"))
